...
def gene_regulatory_network(omega, pdf):
    ...
    gnr.add_reaction(chick.engine.Reaction('P_A_f  + 4*A ->  P_A_A',k_AA_f)) # 6
    gnr.add_reaction(chick.engine.Reaction('P_A_A -> P_A_f + 4*A',k)) # 7
    gnr.add_reaction(chick.engine.Reaction('P_A_f  + B ->  P_A_B',k_AB_f)) # 8
    gnr.add_reaction(chick.engine.Reaction('P_A_B -> P_A_f + B',k)) # 9
    gnr.add_reaction(chick.engine.Reaction('P_A_A  + B ->  P_A_AB',k_AB_f)) # 10
    gnr.add_reaction(chick.engine.Reaction('P_A_AB -> P_A_A + B',k)) # 11
    gnr.add_reaction(chick.engine.Reaction('P_A_B  + 4*A ->  P_A_AB',k_AA_f)) # 12
    ...
    gnr.add_reaction(chick.engine.Reaction('P_B_f  + 4*B ->  P_B_B',k_BB_f)) # 20
    gnr.add_reaction(chick.engine.Reaction('P_B_B -> P_B_f + 4*B',k)) # 21
    gnr.add_reaction(chick.engine.Reaction('P_B_f  + A ->  P_B_A',k_BA_f)) # 22
    gnr.add_reaction(chick.engine.Reaction('P_B_A -> P_B_f + A',k)) # 23
    gnr.add_reaction(chick.engine.Reaction('P_B_B  + A ->  P_B_AB',k_BA_f)) # 24
    gnr.add_reaction(chick.engine.Reaction('P_B_AB -> P_B_B + A',k)) # 25
    gnr.add_reaction(chick.engine.Reaction('P_B_A  + 4*B ->  P_B_AB',k_BB_f)) # 26
