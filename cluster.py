...
class Cluster(object):
    ...
    def sort_clusters(self):
        ...
        for i_traj, dtraj in enumerate(self.dtrajs):
            ...
            if self.fuzzy:
                prob_sorted = deepcopy(self.probs[i_traj])
            for ind_old, ind_new in enumerate(inds_sorted):
                dtraj_sorted[dtraj == ind_old] = ind_new
                if self.fuzzy:
                    prob_sorted[:,ind_new] = self.probs[i_traj][:,ind_old]
            self.dtrajs[i_traj] = dtraj_sorted
            if self.fuzzy:
                self.probs[i_traj] = prob_sorted
            i_start, i_end = self.get_index_merged(i_traj)
    ...
    def score(self):
        ...
        for i_cluster in range(self.n_clusters):
            for i_label in range(n_labels):
                ...
                true_positive = 0.0 # samples belonging to i_cluster AND i_label
                for i_traj, dtraj in enumerate(self.dtrajs):
...
class DensityPeaks(Cluster):
    ...
    def get_energy(self, kernel_radius, n_stds_delta, ns_clusters, pdf, n_gaussians = 1, kernel = 'gaussian', n_bins_rho = 100):
        ...
        if kernel == 'gaussian':
            ...
            #self.kernel_matrix = np.zeros((len(self.trajs_merged),len(self.trajs_merged)))
            ...
            #self.kernel_matrix[inds] = skewnorm.pdf(x[inds], -a)
            ...
            #self.kernel_matrix[np.logical_not(samples_to_keep)] = 0.0
            ...
            #self.kernel_matrix *= module_matrix
        ...
        for i_sample in range(self.n_frames()):
            ...
            if delta_norm_log[i_sample] > f_means(rho_norm_log[i_sample]):
                ...
                probs[i_sample] =  0.5*(1.0 - erf( float((x - mu) / (np.sqrt(2)*sigma) ) ) )
                if n_stds_delta is not None:
                ...
                if np.min(np.abs(rho_norm_log[i_sample] - rho_bins)) < (rho_edges[1]-rho_edges[0]):
                    ...
                    for i_gauss in range(gmm.n_components):
                        ...
                        x = delta_norm_log[i_sample]
                        hp = erf(float(( (x - mu) / (np.sqrt(2)*sigma) )))
            ...
            if self.n_frames_traj(traj_internal) == len(traj):
                if np.all(np.prod(traj_internal == traj, axis = 1).astype(bool)): # this is the internal trajectory
                    ...
                    if self.fuzzy:
                        ...
                        prob = prob / np.sum(prob, axis = 1).reshape((self.n_frames(),1))
...
if __name__ == '__main__':
    ...
    X, y = make_data('1d_gate', 1000, 100)
    #X, y = make_data('moons+', 1000, 100)

    C = DensityPeaks(trajs = X, labels = y, fuzzy = True, verbose = 2)
